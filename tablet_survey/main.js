...
async function init() {
  ...
  if (els.cameraAlignmentPreview) {
    els.cameraAlignmentPreview.addEventListener("click", () => playVideoElement(els.cameraAlignmentPreview));
    els.cameraAlignmentPreview.addEventListener("touchstart", () => playVideoElement(els.cameraAlignmentPreview), { passive: true });
  }
...
async function requestCamera({ showAlert = true } = {}) {
  ...
  try {
    ...
    playVideoElement(els.cameraPreview);
    syncCameraPreview();
...
function playVideoElement(videoEl) {
  ...
  playPromise.catch((error) => {
    console.warn("Camera preview playback was blocked.", error);
    if (els.cameraPreviewHint && videoEl === els.cameraAlignmentPreview) {
      els.cameraPreviewHint.textContent = "摄像头已开启，但当前浏览器阻止了预览自动播放。请轻触预览区域或刷新后重试。";
    }
  });
}
